...
export default function LeaveRequestPage() {
  ...
  return (
    <div className="p-6 max-w-2xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Leave Request</h1>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <textarea
                ...
                className="px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-600 text-sm text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-800 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none"
              />
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
                  {['Date', 'Reason', 'Status'].map(h => <th key={h} className="px-6 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {leaves.map(l => (
                  <tr key={l.id} className="border-b border-slate-700/30 hover:bg-slate-50/50 dark:hover:bg-slate-700/30">
                    <td className="px-6 py-3 font-medium text-slate-900 dark:text-slate-100">{format(parseISO(l.date), 'dd MMM yyyy')}</td>
                    <td className="px-6 py-3 text-slate-600 dark:text-slate-400">{l.reason}</td>
