  ...
  return (
    <div className="p-6 max-w-4xl">
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          {[{ label: 'Senior Doctors', list: seniors, grade: 'SENIOR' }, { label: 'Junior Doctors', list: juniors, grade: 'JUNIOR' }].map(({ label, list }) => (
            <Card key={label}>
              <CardHeader>
                <h2 className="font-semibold text-slate-900">{label} <span className="text-slate-400 font-normal text-sm">({list.length})</span></h2>
              </CardHeader>
                    ...
                    <tbody>
                      {list.map((doc) => (
                        <tr key={doc.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                          <td className="px-6 py-3 font-medium text-slate-900">{doc.name}</td>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Add Doctor">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <select value={form.grade} onChange={e => setForm({...form, grade: e.target.value})} className="px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white">
      ...
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} title="Remove Doctor">
        <p className="text-slate-600 text-sm mb-4">Are you sure you want to remove this doctor? This action cannot be undone.</p>
