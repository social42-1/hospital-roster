...
export default function Dashboard() {
  ...
  const stats = [
    { label: 'Total Doctors', value: doctors?.length ?? '—', icon: Users, color: 'text-blue-600 bg-blue-50' },
    { label: 'Pending Leaves', value: pending, icon: ClipboardList, color: 'text-amber-600 bg-amber-50' },
    { label: 'Published Rosters', value: published, icon: CalendarCheck, color: 'text-emerald-600 bg-emerald-50' },
  ];
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <Card>
        <CardContent>
          <h2 className="font-semibold text-slate-900 mb-3">Quick Access — Rosters</h2>
            ...
            <div className="space-y-2">
              {rosters.slice(0, 6).map(r => (
                <button
                  ...
                  className="flex items-center justify-between w-full px-4 py-3 rounded-lg border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50/30 transition-colors text-sm"
                >
                  <span className="font-medium text-slate-700">
                    {new Date(r.year, r.month - 1).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
                  </span>
                  <div className="flex items-center gap-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${r.status === 'PUBLISHED' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                      {r.status}
                    </span>
