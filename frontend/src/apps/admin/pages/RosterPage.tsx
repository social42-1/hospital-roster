...
import { Roster, Violation, ShiftType } from '@/types';
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <select value={month} onChange={(e) => setMonth(+e.target.value)} className="px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white text-slate-700">
            {months.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
          </select>
          <select value={year} onChange={(e) => setYear(+e.target.value)} className="px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white text-slate-700">
      ...
      {violations.length > 0 && (
        <div className="mb-4 bg-amber-50 border border-amber-200 rounded-xl p-4">
          <button onClick={() => setShowViolations(!showViolations)} className="flex items-center gap-2 text-amber-800 font-medium text-sm w-full text-left">
          ...
          {showViolations && (
            <ul className="mt-3 space-y-1">
              {violations.map((v, i) => <li key={i} className="text-xs text-amber-700">• {v.message}</li>)}
            </ul>
          )}
        </div>
      )}
      ...
      {roster?.status === 'DRAFT' && (
        <div className="mb-4 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-700 font-medium flex items-center gap-2">
          <AlertTriangle className="w-4 h-4" />
      ...
      {isLoading ? (
        ...
        <RosterGrid roster={roster} onShiftClick={setEditTarget} />
      ) : (
      ...
      {editTarget && (
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            {(['MORNING', 'NIGHT', 'OFF', 'WO'] as ShiftType[]).map((type) => {
              ...
              return (
                ...
                >
                  {type === 'MORNING' ? 'Morning' : type === 'NIGHT' ? 'Night' : type === 'OFF' ? 'Day Off' : 'Weekend Off'}
